...
#[tokio::test]
async fn on_before_render_test() {
    let service = create_app()
        .layout(root_layout)
        .hooks(Hooks::new().on_before_render(|html: String, _| async move {
            Ok(html.replace("body-marker", "hashira-123"))
        }))
        .build();
    ...
    assert!(html.contains("hashira-123"));
